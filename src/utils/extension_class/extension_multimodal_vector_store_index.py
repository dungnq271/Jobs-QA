...
class MultiModalVectorStoreIndexExtension(VectorStoreIndexExtension):
    """Multi-Modal Vector Store Index.
    ...
    def __init__(
        ...
        index_struct: MultiModelIndexDict | None = None,
        ...
        # image_vector_store going to be deprecated.
        # image_store can be passed from storage_context
        ...
        # is_image_vector_store_empty is used to indicate
        # whether image_vector_store is empty
    ...
    def as_query_engine(
        self, llm: LLMType | None = None, **kwargs: Any
    ) -> BaseQueryEngine:
        """As query engine."""
        from llama_index.core.indices.multi_modal.retriever import (
            MultiModalVectorIndexRetriever,
    ...
    def _get_node_with_embedding(
        ...
        for node in nodes:
            ...
            if is_image and id_to_text_embed_map:
                ...
                # TODO: re-factor to make use of both embeddings
    ...
    async def _aget_node_with_embedding(
        ...
        for node in nodes:
            ...
            if is_image and id_to_text_embed_map:
                text_embedding = id_to_text_embed_map[node.node_id]
                result.text_embedding = text_embedding
                # TODO: re-factor to make use of both embeddinsg
                result.embedding = text_embedding
            results.append(result)
        return results
    ...
    async def _async_add_nodes_to_index(
        ...
        **insert_kwargs: Any,
        ...
        if not self._vector_store.stores_text or self._store_nodes_override:
            for node, new_id in zip(all_nodes, all_new_ids, strict=False):
    ...
    def delete_extension(
        ...
        **delete_kwargs: Any,
        ...
        self._image_vector_store.delete_extension(value=value, key=key, **delete_kwargs)
    ...
    def filter_extension(self, value: str, key: str = "file_path"):
        ...
        return [
            self._vector_store.filter_extension(value=value, key=key),
            self._image_vector_store.filter_extension(value=value, key=key),
        ]
